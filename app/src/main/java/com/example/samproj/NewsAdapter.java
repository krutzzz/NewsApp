...
import com.example.samproj.model.Body;
...
public class NewsAdapter extends ArrayAdapter<Result> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (listItem == null) {
            listItem = LayoutInflater.from(mContext).inflate(R.layout.articles_cardview, parent, false);
        }
        ...
        String ArtcileString=currentNewsBody.getBodyHtml();
        // The summary of the body text will be used when the CardView is retracted
        final String bodyTextSummaryShort = bodyTextFull.substring(0, Math.min(bodyTextFull.length(), ArticlesFragment.BODY_SUMMARY_TEXT_LENGTH_SHORT)).concat("...");
        final String bodyTextSummaryLong = bodyTextFull.substring(0, Math.min(bodyTextFull.length(), ArticlesFragment.BODY_SUMMARY_TEXT_LENGTH_LONG)).concat("...");
